// lib/api/services/omc-service.ts
...
export class OMCService extends BaseService {
  /**
   * Get paginated OMCs with filtering
   */
  ...
  /**
   * Create OMC with admin user
   */
  async createOMCWithAdmin(omcData: any): Promise<ApiResponse<any>> {
    return this.handleRequest(
      'create_omc_with_admin',
      async () => {
        const user = await this.requireAuth();
        const userProfile = await this.getUserProfile(user.id);
        ...
        if (omcError) {
          throw new Error(`Failed to create OMC: ${omcError.message}`);
        }

        await this.logActivity('omc_created_with_admin', { 
          omc_id: omc.id, 
          omc_name: omc.name 
        });
        ...
        return {
          omc,
          adminUser: omcData.admin_email ? { success: true, message: 'OMC admin user creation pending' } : null
        };
      },
      { requireAuth: true, logActivity: true }
    );
  ...
  /**
   * Update OMC information (Admin only)
   */
  ...
  /**
   * Get OMC dashboard with comprehensive data
   */
